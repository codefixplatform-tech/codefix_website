      ...
      <section className={`relative overflow-hidden ${isDashboard ? 'py-10' : 'pt-24 pb-16 md:pt-36 md:pb-24 lg:pt-44 lg:pb-32'}`}>
        ...
        <div className="max-w-[1400px] mx-auto px-8 sm:px-16 lg:px-24 text-center space-y-8 sm:space-y-10">
           <motion.div {...fadeIn} className="inline-flex items-center gap-3 bg-slate-900 text-white px-4 py-1.5 sm:px-6 sm:py-2 rounded-full border border-white/10 shadow-2xl max-w-full">
              <FaPaperPlane className="text-primary text-[10px] shrink-0" />
              <span className="text-[8px] sm:text-[10px] font-black tracking-[2px] sm:tracking-[4px] uppercase truncate max-w-[180px] xs:max-w-none">Initialize Support Protocol</span>
           </motion.div>
            ...
            <div className="grid lg:grid-cols-12 gap-12 lg:gap-20">
               ...
               <div className="lg:col-span-5 space-y-8 sm:space-y-12">
                  <div className="space-y-6 sm:space-y-8">
                     ...
                     <h2 className="text-4xl sm:text-5xl md:text-6xl font-bold font-heading text-slate-900 leading-tight tracking-tight">Support <br /><span className="bg-gradient-to-r from-primary via-indigo-600 to-purple-600 bg-clip-text text-transparent">Nodes.</span></h2>
               ...
               <div className="lg:col-span-7 relative group w-full">
                   ...
                   <div className="relative bg-white border border-slate-200 p-6 sm:p-12 md:p-20 rounded-[2rem] sm:rounded-[3rem] md:rounded-[4rem] shadow-sm hover:shadow-2xl transition-all duration-700 overflow-hidden">
                      ...
                      <form onSubmit={handleSubmit} className="space-y-8 sm:space-y-12 relative z-10">
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
                            <div className="space-y-4">
                               ...
                               <input 
                                  ...
                                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 px-6 sm:py-6 sm:px-8 text-slate-900 focus:border-primary/50 focus:bg-white outline-none transition-all font-bold text-sm placeholder:text-slate-400 shadow-inner"
                               />
                            </div>
                            <div className="space-y-4">
                         ...
                         <div className="space-y-4">
                            ...
                            <div className="relative">
                               <div 
                                  ...
                                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 px-6 sm:py-6 sm:px-8 text-slate-900 flex items-center justify-between cursor-pointer hover:border-slate-200 transition-all shadow-inner"
                               >
                              ...
                              <AnimatePresence>
                                  {isDropdownOpen && (
                                     <motion.div 
                                        ...
                                        exit={{ opacity: 0, y: 10 }}
                                        className="absolute top-full left-0 right-0 z-50 bg-white border border-slate-200 rounded-[1.5rem] sm:rounded-[2.5rem] p-4 shadow-3xl overflow-hidden"
                                     >
                                       {subjects.map(sub => (
                                           <button 
                                              key={sub.id}
                         ...
                         <div className="space-y-4">
                            ...
                            <textarea 
                               ...
                               className="w-full bg-slate-50 border border-slate-100 rounded-[1.5rem] sm:rounded-[3rem] py-4 px-6 sm:py-8 sm:px-10 text-slate-900 focus:border-primary/50 focus:bg-white outline-none transition-all font-bold text-sm placeholder:text-slate-400 shadow-inner resize-none leading-relaxed"
                            />
                         </div>
                         ...
                         <div className="pt-6">
                            <button 
                               ...
                               className={`w-full bg-slate-900 hover:bg-primary text-white py-4 sm:py-6 md:py-8 rounded-[1.2rem] sm:rounded-[2rem] font-black transition-all hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center gap-3 sm:gap-5 uppercase tracking-[2px] sm:tracking-[5px] text-[10px] sm:text-xs shadow-2xl ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}
                            >
                               {isSubmitting ? (
...
const SupportCard = ({ icon, title, desc, color }) => {
  ...
  return (
    <div className="p-6 sm:p-8 md:p-10 rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3.5rem] bg-white border border-slate-200 hover:border-primary transition-all duration-700 group flex items-start gap-4 sm:gap-8 shadow-sm hover:shadow-2xl text-slate-900 relative overflow-hidden">
      <div className="absolute top-0 right-0 p-6">
        <div className="w-20 h-20 bg-primary/5 blur-3xl rounded-full group-hover:bg-primary/10 transition-all"></div>
      </div>
      <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-[1.5rem] flex items-center justify-center text-2xl sm:text-3xl shrink-0 group-hover:bg-primary group-hover:text-white transition-all duration-700 border ${colors[color]}`}>
        {icon}
      </div>
...
const SocialBtn = ({ icon, label }) => (
  <button 
    ...
    className="flex items-center gap-4 px-5 py-3 sm:px-8 sm:py-4 bg-slate-50 border border-slate-200 rounded-[1.2rem] sm:rounded-[1.5rem] hover:bg-slate-900 hover:text-white transition-all font-black text-[10px] text-slate-500 uppercase tracking-[4px] shadow-sm active:scale-95"
  >
