...
- **Cloud-Powered Engine (Google Drive API v4)**: For complex Office formats, CodeFix securely routes files to a custom **Google Apps Script API**.
  - **High-Fidelity**: Perfect layout preservation for `Word ↔ PDF`, `Excel → PDF`, and `PPTX → PDF`.
...
CodeFix is engineered with a **Search-First** architecture to ensure maximum visibility and high-performance indexing.

### 1. 🔍 Meta Management & Discovery
- **High-Entropy Metadata**: Dynamically optimized `title` and `description` tags tailored for developer-centric keywords.
- **Social Graph Optimization**: Full integration of **Open Graph (OG)** and **Twitter Cards**, ensuring rich previews when shared on platforms like LinkedIn, Twitter, and GitHub.
- **Automated Sitemaps**: Pre-configured `sitemap.xml` ensures that search engine crawlers (Googlebot, Bingbot) can efficiently index the Community QA Hub and Tool pages.
- **Robots Governance**: Optimized `robots.txt` to prioritize high-value user pages while protecting private dashboard routes.

### 2. 🏗️ Semantic Architecture
- **HTML5 Compliance**: Precise usage of semantic elements (`<main>`, `<article>`, `<header>`, `<nav>`) to help search engines understand the information hierarchy.
- **Heading Hierarchy**: Systematic `H1` through `H6` structure across all pages for improved crawlability and user readability.
- **Lighthouse Ready**: Built on Vite + React 19, achieving near-perfect scores for **Core Web Vitals** (LCP, FID, CLS).

### 3. 📱 Mobile-First & Performance
- **Fully Responsive**: Fluid layouts that pass Google's Mobile-Friendly Test with 100% compliance.
- **Fast Interactive (FID)**: Zero-server processing for local tools significantly reduces latency, a key ranking factor for SEO.
- **Progressive Web App (PWA)**: Manifest-enabled for local installation, improving user retention and engagement metrics.
